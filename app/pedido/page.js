...
const OrderPage = () => {
    ...
    const handleClick = async () => {
        ...
        try {
            const printerResponse = await fetch('http://10.2.3.45:5000/Printer_Script', { // Mude pro IP do PC
                method: 'POST',
                body: formData
            });
    
            if (printerResponse.ok) {
                const dbResponse = await fetch('http://10.2.3.45:5000/DB_Script', { // Mude pro IP do PC
                    method: 'POST',
